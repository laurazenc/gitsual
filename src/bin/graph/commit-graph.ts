...
export class CommitGraph {
    ...
    computePositions(repo: RepoBuilder) {
        function insertCommit(commitSha: string, j: number, forbiddenIndices: Set<any>) {
            ...
            while (j - dj >= 0 || j + dj < branches.length) {
                ...
                if (j - dj >= 0 && branches[j - dj] === null && !forbiddenIndices.has(j - dj)) {
                    branches[j - dj] = commitSha
                    return j - dj
                }
                ++dj
    ...
    updateIntervalTree(repo: RepoBuilder) {
